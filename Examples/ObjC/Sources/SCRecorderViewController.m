...
//	_recorder.captureSessionPreset = [SCRecorderTools bestCaptureSessionPresetForDevicePosition:AVCaptureDevicePositionBack withMaxSize:CGSizeMake(3840, 3840)];
	NSError *formatError;
	[_recorder setActiveFormatWithFrameRate:60 width:1080 andHeight:1920 error:&formatError];
	_recorder.autoSetVideoOrientation = NO; //YES causes bad orientation for video from camera roll
//	_recorder.automaticallyConfiguresApplicationAudioSession = YES;
//    _recorder.maxRecordDuration = CMTimeMake(10, 1);
//    _recorder.fastRecordMethodEnabled = YES;
	SCVideoConfiguration *videoConfiguration = _recorder.videoConfiguration;
...
//	videoConfiguration.profileLevel = AVVideoProfileLevelH264HighAutoLevel;
//	videoConfiguration.profileLevel = (__bridge NSString *)kVTProfileLevel_H264_High_5_2;
	videoConfiguration.profileLevel = (__bridge NSString *)kVTProfileLevel_HEVC_Main_AutoLevel;
//	videoConfiguration.profileLevel = (__bridge NSString *)kVTProfileLevel_HEVC_Main10_AutoLevel;
//	    videoConfiguration.timeScale = 1;
	...
	audio.preset = SCPresetHighestQuality;
	// Whether the audio should be enabled or not
	//should already be enabled, changing it to yes causes a crash
	//    audio.enabled = YES;
	// the bitrate of the audio output
//	audio.bitrate = kBITLocalAudioBitrate;
	// Number of audio output channels
	audio.channelsCount = 2; // Mono output
	// The sample rate of the audio output
//	audio.sampleRate = kBITLocalAudioSampleRate;
	...
	[_recorder setMirrorOnFrontCamera:YES];
...
//    [self.recordView addGestureRecognizer:[[SCTouchDetector alloc] initWithTarget:self action:@selector(handleTouchDetected:)]];
	...
	[self.recordView addGestureRecognizer:self.longPressGestureRecognizer];
...
- (void)recorder:(SCRecorder *__nonnull)recorder didAcquireAudioBuffer:(SInt16*)audioBuffer
		  length:(CMItemCount)length timestamp:(CMTime)time {
}

- (void)longPressed:(UILongPressGestureRecognizer *)gesture {
//	NSLog(@"long press with state %i", gesture.state);
