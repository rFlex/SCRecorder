...
- (void)saveToCameraRoll {
	AVAssetTrack * videoAssetTrack = [self.recordSession.assetRepresentingSegments tracksWithMediaType:AVMediaTypeVideo].firstObject;
	NSLog(@"FPS is  : %f ", videoAssetTrack.nominalFrameRate);

	[self.recordSession mergeSegmentsUsingPreset:AVAssetExportPresetPassthrough
...
- (void)saveToCameraRollForURL:(NSURL *)url {
	...
	exportSession.videoConfiguration.size = CGSizeMake(3840, 2160);
	exportSession.audioConfiguration.preset = SCPresetHighestQuality;
	exportSession.videoConfiguration.maxFrameRate = 35;

	NSURL *documentsDirUrl = [NSURL fileURLWithPath:NSTemporaryDirectory() isDirectory:YES];
	NSString *fileName = [NSString stringWithFormat:@"this-export.mp4"];
	__block NSURL *out = [NSURL URLWithString:fileName relativeToURL:documentsDirUrl];

	NSFileManager *fileManager = [NSFileManager defaultManager];
	[fileManager removeItemAtURL:out error:nil];

	exportSession.outputUrl = out;
	...
	[exportSession exportAsynchronouslyWithCompletionHandler:^{
		...
		if (exportSession.cancelled) {
			...
			[out saveToCameraRollWithCompletion:^(NSString * _Nullable path, NSError * _Nullable error) {
